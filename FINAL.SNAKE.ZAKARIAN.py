...
def main():
    # Create canvas and initial settings
    dudraw.set_canvas_size(400, 400)
    grid_size = 20
    dudraw.set_x_scale(0, grid_size)
    dudraw.set_y_scale(0, grid_size)
    
    # Display "Press 'p' to play Snake!"
    dudraw.clear(dudraw.WHITE)
    dudraw.set_pen_color(dudraw.BLACK)
    dudraw.text(10, 10, "Press 'p' to play Snake!")
    dudraw.show()

    # Wait until 'p' is pressed to start the game
    while True:
        if dudraw.has_next_key_typed():
            key = dudraw.next_key_typed()
            if key == 'p':
                break  # Exit loop when 'p' is pressed to start the game
    ...
    while True:
        ...
        dudraw.text(10, 17, " 'a' left    'w' up    'd' right   's' down ")
